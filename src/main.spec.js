...
test({
  ...
  fn: () => {
    ...
    return (
      service.state.toDelete === undefined && service.state.toKeep === 'keep'
    );
  },
  args: [],
  expect: true,
...
test({
  ...
  fn: () => {
    ...
    return (
      service.state.nested.inner === 'updated' &&
...
test({
  ...
  fn: () => {
    ...
    return (
      Array.isArray(service.state.items) &&
...
const asyncTest = async () => {
  ...
  if (
    ...
    console.info('Subscriber notified with new and old state.');

    success =
      notificationReceived &&
      /** @type {any} */ (receivedNewState).initial === 'updated' &&
      /** @type {any} */ (receivedOldState).initial === 'value';
  }
...
console.info(
  '🏁 All ObservableService tests completed! Check console for detailed results.'
);
